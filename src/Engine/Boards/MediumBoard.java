...
public class MediumBoard extends Board{
    ...
    //The game in Easy mode has six colors so this is its palette
    RGB[] medPal = new RGB[]{new RGB(0,0,255), //blue
    ...
    //In Medium Mode, we have 12 trials each of them is a guess, so we have 4 slots for 12 trials. Each slots
    ...
    int[][][] SlotsPositions = new int[12][4][2]; //0: x, 1: y
    ...
    //In Medium Mode, we have 12 trials each of them have a feedback of the guess, so we have 4 slots for 12 trials. Each slots
    //needs two values to identify the position of the centre of the ellipse. The positions will be computed
    //using ComputeFeedbackSlotPositions() below.
    int[][][] FeedbackSlotsPositions = new int[12][4][2]; //0: x, 1: y
    ...
    private void ComputeSlotPositions(){
        int AreaSlotY = 742;
        float y_ratio = (float) y_size /medBoardImage.height;
        float x_ratio = (float) x_size /medBoardImage.width;
    ...
    public MediumBoard(PApplet sw, WorkFlow wf, GenericPlayBoard pb) {
        ...
        setPalette(medPal); //Set Board Palette
        setImage_board(medBoardImage); //Set Board image_board
        calculateImagePadding(); //Set x,y,x_size and y_size of the Board to fit the image
        ...
        setPalettePositions(medPalPos); //Set the positions of Palette Buttons
