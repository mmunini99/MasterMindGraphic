...
public class EasyBoard extends Board{
    ...
    //The game in Easy mode has only three colors so this is its palette
    RGB[] easyPal = new RGB[]{new RGB(0,0,255), //blue
    ...
    //In Easy Mode, we have 15 trials each of them is a guess, so we have 4 slots for 15 trials. Each slots
    ...
    int[][][] SlotsPositions = new int[15][4][2]; //0: x, 1: y
    ...
    //In Easy Mode, we have 15 trials each of them have a feedback of the guess, so we have 4 slots for 15 trials. Each slots
    //needs two values to identify the position of the centre of the ellipse. The positions will be computed
    //using ComputeFeedbackSlotPositions() below.
    int[][][] FeedbackSlotsPositions = new int[15][4][2]; //0: x, 1: y
    ...
    private void ComputeSlotPositions(){
        int AreaSlotY = 742;
        float y_ratio = (float) y_size /easyBoardImage.height;
        float x_ratio = (float) x_size /easyBoardImage.width;
    ...
    private void calculateImagePadding(){
        if(SW.width>=SW.height && easyBoardImage.width<=SW.width){ //Image wider than taller and less wide than the screen
    ...
    public EasyBoard(PApplet arg1, WorkFlow WF, GenericPlayBoard arg2) {
        super(arg1,WF,arg2); //Board Class Constructor

        setPalette(easyPal); //Set Board Palette
        setImage_board(easyBoardImage); //Set Board image_board
        ...
        setPalettePositions(easyPalPos); //Set the positions of Palette Buttons
